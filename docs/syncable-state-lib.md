...
- `#[sync(with = ...)]`: hook cho adapter/custom codec sau này
...
- `SyncableVec<T>` nhưng `T` không có `#[sync(id)]`
...
- nếu `batch.from_seq < local_seq`, v1 coi đó là duplicate/stale replay và bỏ
  qua theo kiểu idempotent, không apply lần hai và không báo gap
